...
class Trainer(object):
    ...
    def __init__(self, exp_name, model, train_dataloader, val_dataloader, word_to_idx, idx_to_word, learning_rate = 0.001, num_epochs = 10, print_every = 1, verbose = True, device = 'cuda'):
        self.model = model
        ...
        self.ts_writer = SummaryWriter(log_dir=f'runs/{exp_name}')
    ...
    def train(self):
        ...
        for i in range(self.num_epochs):
            ...
            for batch in self.train_dataloader:
                print(f"batch {num_batches} / {len(self.train_dataloader)}")
                features, questions, answers = batch[0].to(self.device), batch[1].to(self.device), batch[2].to(self.device)
                logits = self.model(features, questions, answers[:,:-1])
                loss = self.loss(logits, answers[:, 1:])
            ...
            with open(f'{self.exp_name}_train_loss.npy', 'wb') as f:
            ...
            if self.verbose and (i +1) % self.print_every == 0:
                ...
                f.close()
                self.ts_writer.add_scalar('Loss/val', self.val_loss_history[-1], i)
