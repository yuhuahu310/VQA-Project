...
set_all_seeds(42)
exp_name = 'vit+bert+transformer+attn'
ENC_EMB_DIM = 1000
DEC_EMB_DIM = 256
HID_DIM = 128
N_LAYERS = 4
...
train_dataset = VQA_mm2_Dataset("../../../../net/projects/ranalab/kh310/vqa", "train", img_transforms=pretrained_vit_weights.transforms(), tokenizer=tokenizer)
qa_dataset_val = VQA_mm2_Dataset("../../../../net/projects/ranalab/kh310/vqa", "val", img_transforms=pretrained_vit_weights.transforms(), tokenizer=tokenizer)
...
def train_and_test(param, value):
    exp_name = "vit_bert_att_%s_%d" % (param, value)
    if param == "HID_DIM":
    ...
    torch.save(transformer.state_dict(), f"./model_{exp_name}.pth")
...
#   for value in values:
train_and_test(args.param, args.value)